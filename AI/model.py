...
class DiagnosisPredictor:
    ...
    """
    def __init__(self):
        self.model = RandomForestClassifier(n_estimators=10000000, random_state=15129)
        self.symptoms = []
        self.trained = False
        self.model_path = "diagnosis_model.pkl"
    ...
    def predict(self, symptoms_data):
        ...
        if not self.trained:
            if os.path.exists(self.model_path):
                ...
                raise ValueError("Modelul nu a fost antrenat. Apelați metoda train() înainte de predict().")
        ...
        # Verificăm că avem toate coloanele necesare
    ...
    def _load_model(self):
        ...
        if not os.path.exists(self.model_path):
            raise FileNotFoundError(f"Modelul nu a fost găsit la calea: {self.model_path}")
...
def get_diagnosis_and_medication(symptoms_data, data):
    ...
    if not os.path.exists(predictor.model_path):
        ...
        try:
            ...
            # Dacă există probleme cu încărcarea modelului, reantrenăm
            predictor.train(data)
